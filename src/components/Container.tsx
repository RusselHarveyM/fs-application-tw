export default function Container({
  ...
  return (
    <button
      {...props}
      className="flex transition ease-in delay-50 hover:scale-105 hover:brightness-105 rounded-xl justify-start gap-4 items-center shadow-sm border-2 border-neutral-200 bg-white py-2 px-4 md:w-80 md:h-28 sm:w-60 sm:h-30 xs:w-40 xs:h-25 hover:cursor-pointer"
    >
      {img ? (
        <img
      ...
      <div className="flex flex-col md:gap-4 sm:gap-2">
        <div>
          {title ? (
            <h2 className="w-fit md:text-base sm:text-xs xs:text-xs">
              {title}
            </h2>
          ) : (
